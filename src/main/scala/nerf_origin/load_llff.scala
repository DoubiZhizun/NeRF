...
object load_llff {
  ...
  def load_llff_data(basedir: String, factor: Int = 8, recenter: Boolean = true, bd_factor: Double = .75, spherify: Boolean = false, path_zflat: Boolean = false, manager: NDManager): (NDArray, NDArray, NDArray, NDArray, Array[Int]) = {
    ...
    var (poses, bds, imgs) = _load_data(basedir, factor = factor, manager = subManager)
    ...
    if (spherify) {
      ...
      val rads = subManager.create(percentile(tt.toDoubleArray, tt.getShape.get(0).toInt, 90)).toType(poses.getDataType, false)
      //val rads = tt.percentile(90, 0)
      val c2w_path = c2w
      var N_views = 120
      var N_rots = 2
      if (path_zflat) {
    ...
    val render_poses2 = manager.create(render_poses.getShape, DataType.FLOAT32)
    render_poses.copyTo(render_poses2)
    ...
    val images2 = manager.create(images.getShape, DataType.FLOAT32)
    images.copyTo(images2)
    val poses2 = manager.create(poses.getShape, DataType.FLOAT32)
    poses.copyTo(poses2)
    val bds2 = manager.create(bds.getShape, DataType.FLOAT32)
    bds.copyTo(bds2)

    subManager.close()
    (images2, poses2, bds2, render_poses2, i_test)
  }
}